...
(:action putdown :parameters (?ob) :precondition (and (holding ?ob)) :effect (and (clear ?ob) (arm-empty) (on-table ?ob) (not (holding ?ob))))
...
(:action unstack :parameters (?ob ?underob) :precondition (and (on ?ob ?underob) (clear ?ob) (arm-empty)) :effect (and (holding ?ob) (clear ?underob) (not (on ?ob ?underob)) (not (clear ?ob)) (not (arm-empty)))))
```
...
### Output/Plan:
```
\[image](https://github.com/SamyukthaPM118/AI_Lab_2023-24/assets/160995906/4bc0cf55-ab53-4217-9dda-a6b1ad6401cf)
```
